...
public class StoryController extends Base {
    ...
    public Res deleteStoryContent(@PathVariable long id) {
        mapper().story().delete_story_content(id, "Story deleted.");
    ...
    public Res getStoryContent(@PathVariable long id) {
        return Res.ok(mapper().story().getStoryContentById(id));
    }

    @GetMapping("/by-type")
    public Res getStoryByType(TagType tag) {
        return Res.ok(mapper().story().getStoryByType(tag));
    }
    ...
    public Res collectStory(@PathVariable(name = "id") long storyId, HttpServletRequest request) {
        ...
        mapper().story().collectStory(collect);
        return Res.ok();
    }
    ...
    public Res getCollection(HttpServletRequest request) {
        Res isLogin = service().auth().authenticate(request);
        if (!isLogin.getSuccess()) return isLogin;
        List<Integer> storyIds = mapper().story().getStoryByCollector((Integer) isLogin.getData());
        List<Story> stories = new ArrayList<>();
        for (int storyId : storyIds) {
            stories.add(mapper().story().getStoryById(storyId));
        }
